...
class PlotUpdate extends Equatable {
  ...
  static PlotUpdate fromFilename(String filename) {
    ...
    final parts = filename
        .replaceFirst('realtime_', '')
        .replaceFirst('.png', '')
        .split('_');

    if (parts.length >= 2) {
      parsedVariable = parts[0];
      // Attempt to parse elevation (e.g., "250" -> 2.5)
      if (double.tryParse(parts[1]) != null) {
        parsedElevation = double.parse(parts[1]) / 100.0;
      }
    }
  ...
  factory PlotUpdate.fromJsonString(String jsonString) {
    try {
      ...
      log('Error parsing PlotUpdate JSON string: $e', name: 'PlotUpdate');
    }
    // Return a default/invalid state if parsing fails
    return const PlotUpdate(
