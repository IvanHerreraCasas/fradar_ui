  ...
  Widget build(BuildContext context) {
    return BlocBuilder<HistoricPlotsBloc, HistoricPlotsState>(
      ...
      buildWhen: (prev, current) =>
          ...
          prev.currentFrameImage != current.currentFrameImage || // For carousel update
          prev.frames != current.frames || // For carousel data
          prev.currentFrameIndex != current.currentFrameIndex || // For carousel index
          prev.isPlaying != current.isPlaying || // For carousel controls
          prev.fps != current.fps || // For carousel controls
          prev.videoPlayerController != current.videoPlayerController || // For video
          prev.isVideoControllerInitialized != current.isVideoControllerInitialized, // For video
      builder: (context, state) {
        ...
        if (state.status == HistoricPlotsStatus.loadingFrames ||
          ...
          String message = switch(state.status) {
             ...
             HistoricPlotsStatus.monitoringJob => 'Monitoring Job: ${state.activeJob?.statusDetails ?? '...'}',
             HistoricPlotsStatus.loadingVideo => 'Loading Video Data...',
             HistoricPlotsStatus.initializingVideo => 'Initializing Video Player...',
        ...
        if (state.status == HistoricPlotsStatus.framesLoadSuccess ||
           ...
           if (state.frames.isEmpty && state.status != HistoricPlotsStatus.animationReadyToDownload) {
               return const Center(child: Text('No frames found for the selected parameters.'));
           }
           ...
           return ImageCarousel(
              ...
              canGenerateOrExport: state.canGenerateOrExport, // Pass for enabling export button
           );
        }

        // --- Video Ready (Show Video Player) ---
        if (state.status == HistoricPlotsStatus.videoReady &&
            state.videoPlayerController != null &&
            state.isVideoControllerInitialized) {
           return VideoDisplay(
              controller: state.videoPlayerController!,
              canGenerateOrExport: state.canGenerateOrExport, // Although maybe redundant here
           );
        }
