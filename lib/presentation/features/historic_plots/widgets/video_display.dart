...
class VideoDisplay extends StatefulWidget {
  ...
  const VideoDisplay({super.key, required this.tempVideoFilePath});
...
class _VideoDisplayState extends State<VideoDisplay> {
    ...
    _initializeVideoPlayerFuture =
        _controller!.initialize()
          ..then((_) {
            ...
            if (mounted) setState(() {});
            context.read<HistoricPlotsBloc>().add(
              ErrorOccurred('Video Player Init Error: $error'),
            );
          });
  }
  ...
  void dispose() {
    ...
    _controller?.dispose().then((_) {
      ...
      if (widget.tempVideoFilePath.isNotEmpty) {
        context.read<HistoricPlotsBloc>().add(
          VideoFileReadyToClean(widget.tempVideoFilePath),
        );
      }
    });
    super.dispose();
  ...
  Widget build(BuildContext context) {
    final bloc =
        context.read<HistoricPlotsBloc>(); // Needed for download action
      ...
      children: [
        Expanded(
          child: Container(
            ...
            child: Center(
              child:
                  _initializeVideoPlayerFuture == null || _controller == null
