...
class SettingsBloc extends Bloc<SettingsEvent, SettingsState> {
  SettingsBloc({required RadprocRepository radprocRepository})
      : _radprocRepository = radprocRepository,
        super(const SettingsState()) { // Initial state
  ...
  void _onApiUrlChanged(ApiUrlChanged event, Emitter<SettingsState> emit) {
    ...
    emit(state.copyWith(apiUrl: event.apiUrl, status: SettingsStatus.editing, clearError: true));
  }

  Future<void> _onSaveSettingsClicked(
      SaveSettingsClicked event, Emitter<SettingsState> emit) async {
    // Use the URL currently held in the state
    final urlToSave = state.apiUrl.trim(); // Trim whitespace

    // Optional: Basic URL validation
    if (urlToSave.isEmpty || !(Uri.tryParse(urlToSave)?.hasAbsolutePath ?? false)) {
         emit(state.copyWith(status: SettingsStatus.failure, errorMessage: 'Invalid URL format.'));
         return;
    }
